"""
Attribute: Data series with item size metadata for scalar/vector/tensor properties.
"""
...
class AttributeType:
    """Constants for Attribute semantic types."""
    SCALAR = "scalar"
    VECTOR2 = "vector2"
    VECTOR3 = "vector3"
...
_DEFAULT_TYPES = {
    1: AttributeType.SCALAR,
    2: AttributeType.VECTOR2,
    3: AttributeType.VECTOR3,  # Could also be SYM_TENSOR2
    6: AttributeType.SYM_TENSOR3,
    9: AttributeType.TENSOR3,
}
...
class Attribute:
    ...
    pressure = Attribute([1.0, 2.0, 3.0], item_size=1)
    for p in pressure:
        print(p)  # prints: 1.0, 2.0, 3.0

    # Vector property (item_size=3)
    velocity = Attribute([[1, 2, 3], [4, 5, 6]], item_size=3)
    for v in velocity:
        print(v)  # prints: [1, 2, 3], [4, 5, 6]

    # Symmetric tensor (item_size=6: xx, xy, xz, yy, yz, zz)
    stress = Attribute(np.zeros((10, 6)), item_size=6)
    ...
    Attributes:
        ...
        attribute_type: Semantic type (vector3, sym_tensor3, etc.)
    """
    ...
    def __init__(
        ...
        Args:
            ...
            attribute_type: Semantic type (e.g., AttributeType.VECTOR3). If None, inferred.
        """
        ...
        # Infer or set attribute_type
        if attribute_type is None:
            self._attribute_type = _DEFAULT_TYPES.get(item_size, AttributeType.UNKNOWN)
        else:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Attribute({name_str}, n_items={self.n_items}, "
    ...
    def from_flat(
        ...
        Returns:
            Attribute with data reshaped according to item_size.
        """
        data = np.asarray(data, dtype=np.float64)
        if item_size > 1:
            data = data.reshape(-1, item_size)
        return cls(data, item_size=item_size, name=name, attribute_type=attribute_type)
    ...
    def as_view(
        ...
        Modifications to the Attribute will affect the original array.
        ...
        # Infer or set attribute_type
        if attribute_type is None:
            instance._attribute_type = _DEFAULT_TYPES.get(item_size, AttributeType.UNKNOWN)
        else:
            instance._attribute_type = attribute_type
